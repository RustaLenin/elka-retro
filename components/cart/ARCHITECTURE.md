...
### Действия пользователя (жёсткое обновление сервера)

- `addItem`, `removeItem`, `clearCart` → обновляют LocalStorage **и** вызывают `PUT /wp-json/elkaretro/v1/cart`.
- Сервер сохраняет переданное состояние в user meta без дополнительной логики объединения.
- После успешного запроса LocalStorage и user meta содержат одинаковые данные.

### Действия системы (фоновая синхронизация)

- При загрузке сайта для авторизованного пользователя вызывается `PUT /wp-json/elkaretro/v1/cart/sync`.
- Цель — подтянуть корзину с другого устройства, не перезаписывая серверные данные.
- Приоритет: user meta → LocalStorage. Клиент никогда не затирает сервер в ходе фонового синка.

### Правила

1. **Не используем sync-эндпоинт для действий пользователя.** Он предназначен только для фонового процесса.
2. **Всегда сохраняем актуальное состояние на сервер сразу после действия пользователя.**
3. **После оформления заказа** вызываем очистку: LocalStorage + `PUT /wp-json/elkaretro/v1/cart` с пустой корзиной.
